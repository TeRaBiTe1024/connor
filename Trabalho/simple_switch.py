...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if dst in self.mac_to_port[dpid] or dst == 'ff:ff:ff:ff:ff:ff':
            print("DST in Switch or any")
            if dst == 'ff:ff:ff:ff:ff:ff':
                out_port = 0
                print('KEYS:', self.mac_to_port[dpid].keys())
                for x in self.mac_to_port[dpid].keys():
                    if x != src:
                        out_port = self.mac_to_port[dpid][x]
                        actions.append(parser.OFPActionOutput(out_port))
                if dpid != 1 and dpid != 4:
                    if in_port != 1 and parser.OFPActionOutput(1) not in actions:
                       out_port = 1
                    if in_port != 2 and parser.OFPActionOutput(2) not in actions:
                       out_port = 2
                else:
                    if in_port == 3:
                        out_port = random.choice([1, 2])
                    elif parser.OFPActionOutput(3) not in actions:
                        out_port = 3
            else:
